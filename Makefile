...
juicefs-image-nightly:
	# Build image with newest juicefs-csi-driver and juicefs
	docker build -f juicefs.Dockerfile --build-arg TARGETARCH=amd64 --build-arg JFSCHAN=beta --build-arg JUICEFS_REPO_BRANCH=main
        -t $(ACR_REGISTRY)/$(JUICEFS_IMAGE):nightly .
